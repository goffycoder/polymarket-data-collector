...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build the Phase 9 Task 1 reference-window preparation manifest and summary."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="reports/phase9/reference_window_preparation",
        help="Directory for the generated Phase 9 Task 1 manifest and summary.",
    )
...
def main() -> int:
    ...
    manifest = build_phase9_task1_manifest(start=args.start, end=args.end)

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = output_dir / "phase9_task1_reference_window_manifest.json"
    summary_path = output_dir / "phase9_task1_reference_window_summary.md"

    manifest_sha256 = write_json(manifest_path, manifest)
    summary_sha256 = write_markdown(summary_path, render_phase9_task1_markdown(manifest))

    payload = {
        "reference_window": manifest["reference_window"],
        "overall_readiness": manifest["workspace_availability"]["overall_readiness"],
        "artifacts": {
