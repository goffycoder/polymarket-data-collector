...
from phase3.detector import PHASE3_DETECTOR_VERSION, PHASE3_FEATURE_SCHEMA_VERSION, Phase3Repository
...
def _assessment(
    ...
    checkpoint_count = int(runtime_status.get("checkpoint_count") or 0)

    if checkpoint_count == 0:
        status = "no_persisted_checkpoints"
    elif persisted == 0 and replayed == 0:
...
async def build_phase3_gate3_report(*, start: str, end: str) -> Phase3Gate3Report:
    ...
    return Phase3Gate3Report(
        ...
        assessment=_assessment(runtime_status, candidate_report, reconciliation_report),
    )
