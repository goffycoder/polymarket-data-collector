...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Write the Phase 8 canonical v1 operating-mode decision and policy artifacts."
    )
    parser.add_argument(
        "--output-dir",
        default="reports/phase8/operating_mode",
        help="Directory for the generated operating-mode manifest and summary.",
    )
...
def _render_text(payload: dict[str, object]) -> str:
    return "\n".join(
        [
            f"Canonical v1 mode: {payload['decision']['canonical_v1_operating_mode']}",
...
def main() -> int:
    ...
    manifest = build_v1_operating_mode_manifest()

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = output_dir / "phase8_v1_operating_mode_manifest.json"
    summary_path = output_dir / "phase8_v1_operating_mode_summary.md"

    manifest_sha256 = write_json(manifest_path, manifest)
    summary_sha256 = write_markdown(summary_path, render_v1_operating_mode_markdown(manifest))
